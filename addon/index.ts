...
import { Artisan, JSONRPCResponse, WorkerProxyStrategy } from 'ember-artisans/types';
...
export function createWorker (
  ...
  return new Proxy(artisanInstance, {
    get (target, method, receiver) {
      ...
      return async function taskWrapper (...params: any[]): Promise<JSONRPCResponse> {
...
export function createWorkerPool (workerPath: string, poolSize: number): Artisan[] & WorkerProxyStrategy<Promise<JSONRPCResponse>> {
  const availableWorkers = new Array(poolSize).fill(null).map(
    () => createWorker(workerPath)
  )

  return new Proxy(availableWorkers, {
    get (workerPool, method: string) {
      return function taskPoolWrapper (...params: any[]): Promise<JSONRPCResponse> {
