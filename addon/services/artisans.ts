...
export default class Artisans extends Service {
  private _pools: {
    [workerName: string]: Artisan[] &
      WorkerProxyStrategy<Promise<JSONRPCResponse>>;
  };
  ...
  public poolFor(workerName: string, poolSize: number = 4) {
    const { _pools } = this;
  ...
  public willDestroy(...args: any[]) {
    Object.entries(this._pools).forEach(([_, workerPool]) => {
